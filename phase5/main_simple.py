"""
Phase 5 Simple Main Entry Point: Standalone runner for the presentation layer.

This module provides a standalone entry point that can be run directly
without relative imports and without Unicode issues.
"""
...
def render_enhanced_output(
    ...
    lines.append("ENHANCED RESTAURANT RECOMMENDATIONS")
    lines.append("=" * args.max_width)
    lines.append("")
    ...
    console_output = renderer.render_console_simple(restaurants)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.format == "console":
            output = renderer.render_console_simple(restaurant_displays)
